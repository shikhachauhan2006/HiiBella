...
GEMINI_API_KEY  = "AIzaSyCsXkAoUzFgCXUXGkffHWi9fGtrSYMXeMo"   
...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;font-family:'Segoe UI',Arial,sans-serif}
    ...
    .chat-box{
      width:440px;background:#fff;border-radius:22px;
      box-shadow:0 24px 64px rgba(13,71,161,0.4);
      overflow:hidden;display:flex;flex-direction:column;height:660px;
    }
    ...
    .status-dot{
      width:7px;height:7px;background:#69f0ae;border-radius:50%;
    ...
    .tab-btn{
      ...
      font-size:10.5px;cursor:pointer;transition:all .2s;white-space:nowrap;
    }
    ...
    #chat-panel{
      ...
      display:flex;flex-direction:column;gap:8px;background:#e8f0fe;
    }
    ...
    .typing{
      ...
      width:fit-content;box-shadow:0 1px 5px rgba(0,0,0,.1);
    }
    .typing span{
      width:7px;height:7px;background:#90caf9;
    ...
    /* ── Quick replies ── */
    .quick-replies{
      padding:8px 10px 4px;display:flex;flex-wrap:wrap;gap:5px;
      background:#e8f0fe;border-top:1px solid #bbdefb;flex-shrink:0;
    }
    .qr-btn{
      padding:5px 11px;background:#fff;
      border:1.5px solid #1976d2;color:#1565c0;
    ...
    /* ── Categories ── */
    .cat-label{
      font-size:10px;color:#90a4ae;padding:5px 14px 2px;
      background:#e8f0fe;font-weight:700;
      letter-spacing:.05em;text-transform:uppercase;flex-shrink:0;
    }
    .categories{
      padding:4px 10px 8px;display:flex;flex-wrap:wrap;gap:5px;
      background:#e8f0fe;border-bottom:1px solid #bbdefb;flex-shrink:0;
    }
    .cat-btn{
      padding:6px 10px;background:#fff;border:1px solid #bbdefb;
      color:#444;border-radius:12px;cursor:pointer;transition:all .2s;
    ...
    .cat-btn:hover{
      ...
      transform:translateY(-2px);box-shadow:0 3px 8px rgba(21,101,192,.2);
    }
    ...
    .product-card{
      background:#fff;border:1px solid #bbdefb;border-radius:14px;
      padding:10px;cursor:pointer;transition:all .2s;
      display:flex;flex-direction:column;align-items:center;gap:5px;
      text-align:center;
    }
    .product-card:hover{
      border-color:#1976d2;transform:translateY(-2px);
      box-shadow:0 4px 12px rgba(21,101,192,.2);background:#e3f2fd;
    }
    ...
    .buy-now-btn{
      ...
      cursor:pointer;width:100%;margin-top:3px;transition:transform .2s;
    }
    .buy-now-btn:hover{transform:scale(1.03)}

    /* Buy steps */
    .buy-title{
      font-size:13.5px;font-weight:700;color:#0d47a1;
      margin:4px 0 10px;text-align:center;padding:10px;
      background:#fff;border-radius:12px;border:1px solid #bbdefb;
    }
    .buy-step{
      display:flex;gap:11px;align-items:flex-start;
      background:#fff;border-radius:13px;padding:11px;
      margin-bottom:7px;border:1px solid #e3f2fd;
    ...
    .buy-help-box{
      background:#e3f2fd;border-radius:13px;padding:12px;
      margin-top:8px;text-align:center;border:1px solid #90caf9;
    }
    .buy-ask-btn{
      background:linear-gradient(135deg,#1976d2,#0d47a1);
    ...
    .sell-earn-box{
      background:#e8f5e9;border-radius:13px;padding:12px;
      margin-top:8px;text-align:center;border:1px solid #a5d6a7;
    }

    /* ── Input ── */
    .chat-input{
      display:flex;border-top:1px solid #e3f2fd;padding:10px 12px;
      gap:8px;background:#fff;align-items:center;flex-shrink:0;
    }
    .chat-input input{
      flex:1;padding:10px 14px;border:1.5px solid #bbdefb;
      border-radius:24px;font-size:13.5px;outline:none;
      transition:border .2s;background:#e8f0fe;
    }
...
<div class="chat-box">
  ...
  <div id="buy-panel">
    ...
    <div class="product-cards">
      ...
      <div class="product-card" onclick="send('show me Java projects')">
        <div class="product-card-icon">☕</div>
        <div class="product-card-name">Java Projects</div>
        <div class="product-card-price">From ₹299</div>
        <div class="product-card-tag">Spring • JSP • JDBC</div>
        <button class="buy-now-btn">🛒 Browse</button>
      </div>

      <div class="product-card" onclick="send('show me Python projects')">
        <div class="product-card-icon">🐍</div>
        <div class="product-card-name">Python Projects</div>
        <div class="product-card-price">From ₹299</div>
        <div class="product-card-tag">Django • Flask • FastAPI</div>
        <button class="buy-now-btn">🛒 Browse</button>
      </div>

      <div class="product-card" onclick="send('show me AI/ML projects')">
        <div class="product-card-icon">🤖</div>
        <div class="product-card-name">AI/ML Projects</div>
        <div class="product-card-price">From ₹499</div>
        <div class="product-card-tag">TensorFlow • scikit-learn</div>
        <button class="buy-now-btn">🛒 Browse</button>
      </div>

      <div class="product-card" onclick="send('show me Web Development projects')">
        <div class="product-card-icon">🌐</div>
        <div class="product-card-name">Web Dev Projects</div>
        <div class="product-card-price">From ₹199</div>
        <div class="product-card-tag">React • HTML • Node.js</div>
        <button class="buy-now-btn">🛒 Browse</button>
      </div>

      <div class="product-card" onclick="send('show me Final Year projects')">
        <div class="product-card-icon">🎓</div>
        <div class="product-card-name">Final Year</div>
        <div class="product-card-price">From ₹799</div>
        <div class="product-card-tag">Full source + docs</div>
        <button class="buy-now-btn">🛒 Browse</button>
      </div>

      <div class="product-card" onclick="send('show me Mini projects')">
        <div class="product-card-icon">🔧</div>
        <div class="product-card-name">Mini Projects</div>
        <div class="product-card-price">From ₹99</div>
        <div class="product-card-tag">Quick & easy</div>
        <button class="buy-now-btn">🛒 Browse</button>
      </div>
    ...
    <div class="buy-step">
      <div class="buy-icon" style="background:#e3f2fd">🔍</div>
      <div>
        <div class="buy-step-title">Step 1 — Search a project</div>
        <div class="buy-step-desc">Click any product card above or type in chat. Example: "show me Python projects"</div>
      </div>
    </div>
    <div class="buy-step">
      <div class="buy-icon" style="background:#e8f5e9">📋</div>
      <div>
        <div class="buy-step-title">Step 2 — View project details</div>
        <div class="buy-step-desc">Ask Bella for price, tech stack, difficulty and description of any project.</div>
      </div>
    </div>
    <div class="buy-step">
      <div class="buy-icon" style="background:#fffde7">🛒</div>
      <div>
        <div class="buy-step-title">Step 3 — Add to cart & login</div>
        <div class="buy-step-desc">Click "Buy Now" on the project page. Register or login as a buyer.</div>
      </div>
    </div>
    <div class="buy-step">
      <div class="buy-icon" style="background:#fff8e1">💳</div>
      <div>
        <div class="buy-step-title">Step 4 — Make payment</div>
        <div class="buy-step-desc">Pay via UPI, Credit/Debit card, or Net Banking. 100% secure!</div>
      </div>
    </div>
    <div class="buy-step">
      <div class="buy-icon" style="background:#f3e5f5">📥</div>
      <div>
        <div class="buy-step-title">Step 5 — Download instantly</div>
        <div class="buy-step-desc">Go to My Orders → click project → Download. Get source code + docs!</div>
      </div>
    </div>
    <div class="buy-step">
      <div class="buy-icon" style="background:#e0f7fa">⭐</div>
      <div>
        <div class="buy-step-title">Step 6 — Leave a review</div>
        <div class="buy-step-desc">Rate the project and help other buyers make better decisions!</div>
      </div>
    </div>

    <div class="buy-help-box">
      <div style="font-size:13px;font-weight:700;color:#0d47a1;margin-bottom:5px">💬 Need help buying?</div>
      <div style="font-size:12px;color:#546e7a;margin-bottom:10px">Ask Bella — she's always here!</div>
      <button class="buy-ask-btn" onclick="showTab('chat');send('how do I buy a project')">Ask Bella →</button>
    </div>
  </div>
  ...
  <div id="sell-panel">
    ...
    <div class="sell-step">
      <div class="sell-icon" style="background:#e3f2fd">📝</div>
      <div>
        <div class="sell-step-title">Step 1 — Register as seller</div>
        <div class="sell-step-desc">Sign up on Project Marketplace and choose "Seller" as your account type.</div>
      </div>
    </div>
    <div class="sell-step">
      <div class="sell-icon" style="background:#e8f5e9">📁</div>
      <div>
        <div class="sell-step-title">Step 2 — Upload your project</div>
        <div class="sell-step-desc">Dashboard → Upload Project. Add title, description, tech stack and price.</div>
      </div>
    </div>
    <div class="sell-step">
      <div class="sell-icon" style="background:#fff8e1">📦</div>
      <div>
        <div class="sell-step-title">Step 3 — Add ZIP file + docs</div>
        <div class="sell-step-desc">Upload source code as ZIP. Include README with setup instructions.</div>
      </div>
    </div>
    <div class="sell-step">
      <div class="sell-icon" style="background:#fce4ec">🔍</div>
      <div>
        <div class="sell-step-title">Step 4 — Submit for review</div>
        <div class="sell-step-desc">Our team reviews within 24 hours and approves or gives feedback.</div>
      </div>
    </div>
    <div class="sell-step">
      <div class="sell-icon" style="background:#e0f7fa">🌐</div>
      <div>
        <div class="sell-step-title">Step 5 — Go live & earn!</div>
        <div class="sell-step-desc">Once approved, buyers find and buy your project. You earn 80% of every sale!</div>
      </div>
    </div>
    <div class="sell-step">
      <div class="sell-icon" style="background:#e8f5e9">💰</div>
      <div>
        <div class="sell-step-title">Step 6 — Get paid</div>
        <div class="sell-step-desc">Payments every Monday via UPI or bank transfer. Minimum payout: ₹500.</div>
      </div>
    </div>

    <div class="sell-earn-box">
      <div style="font-size:13px;font-weight:700;color:#2e7d32;margin-bottom:5px">💰 You earn 80% of every sale!</div>
      <div style="font-size:12px;color:#546e7a;margin-bottom:10px">Example: ₹499 project → You get ₹399!</div>
      <button class="buy-ask-btn" onclick="showTab('chat');send('how do I sell my project')">Ask Bella how to sell →</button>
    </div>
  </div>
  ...
  <div class="categories" id="cat-btns">
    <button class="cat-btn" onclick="send('show me Java projects')">
      <span class="cat-icon">☕</span><span>Java</span>
    </button>
    <button class="cat-btn" onclick="send('show me Python projects')">
      <span class="cat-icon">🐍</span><span>Python</span>
    </button>
    <button class="cat-btn" onclick="send('show me AI/ML projects')">
      <span class="cat-icon">🤖</span><span>AI/ML</span>
    </button>
    <button class="cat-btn" onclick="send('show me Web Development projects')">
      <span class="cat-icon">🌐</span><span>Web Dev</span>
    </button>
    <button class="cat-btn" onclick="send('show me Final Year projects')">
      <span class="cat-icon">🎓</span><span>Final Year</span>
    </button>
    <button class="cat-btn" onclick="send('show me Mini projects')">
      <span class="cat-icon">🔧</span><span>Mini</span>
    </button>
  </div>
...
<script>
  ...
  async function sendMessage() {
    ...
    try {
      ...
      data.responses.forEach(r => {
        appendMessage(r.text || JSON.stringify(r), 'bot', data.handled_by);
      });
    } catch (e) {
      removeTyping(typingEl);
  ...
  function appendMessage(text, sender, handledBy = '') {
    ...
    if (sender === 'bot' && handledBy) {
      const badge = document.createElement('div');
      badge.className = 'badge';
      badge.innerText = handledBy === 'gemini_fallback' ? '✨ Bella (AI)' : '💙 Bella';
  ...
  function removeTyping(el) {
    ...
    const b = document.getElementById('chat-panel');
    if (b) b.scrollTop = b.scrollHeight;
  }

  function showTab(tab) {
    ['chat-panel','buy-panel','sell-panel'].forEach(id => {
      document.getElementById(id).style.display = 'none';
    });
    ['tab-chat','tab-buy','tab-sell'].forEach(id => {
...
async def health():
    try:
        ...
        rasa_status = "down"
    return {"bella": "up", "rasa": rasa_status, "ai_fallback": "gemini-1.5-flash (free)"}
